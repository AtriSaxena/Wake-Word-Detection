...
# def arg_parse():
#     parser = argparse.ArgumentParser(description="Wake-Word create features")
...
class Data2Features:
    def __init__(self, target_class:str, data_path):
        ...
        self.target_label = 0
        self.non_target_label = 1
        self.features = [] 
        self.classes = os.listdir(self.data_path).remove("_background_noise_")
        self.non_target_class = ['seven'] #self.classes.remove(self.target_class)
    ...
    def create_features(self):
        ...
        return create_train_test_split(self.features[:,0], self.features[:,1])
