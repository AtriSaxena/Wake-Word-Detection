...
import argparse
from data_loader import torch_dataloader 
from models.model import WakeWordModel
import sys
import os
from pathlib import Path 
from data_to_features import Data2Features
...
def train(X_train, y_train, X_test, y_test, EPOCHS, BATCH_SIZE, DEVICE, seed, OPTIMIZER):
    ...
    for epoch in tqdm(range(EPOCHS)): 
        ...
        with torch.inference_mode(): 
            for X,y in test_dataloader:
                ...
                #3. Test Accuracy
                accuracy, precision, recall, f1score =  evaluate(test_pred.squeeze(dim=1), y) 
    ...
    torch.save(model.state_dict(), "WakeWordDetection.pth")


def arg_parse():
    parser = argparse.ArgumentParser() 
    parser.add_argument("--dataset", type=str, default= ROOT / 'dataset/', help='Dataset path')
    parser.add_argument('--feature_file', type=str, default=ROOT / 'features/wakeword_features.npz')
...
def main(arg_out):
    ...
    if not(Path(arg_out.feature_file).is_file()):
        ...
        np.savez(ROOT / "features/wakeword_features", X_train = X_train, 
                            X_test = X_test,
                            y_train = y_train,
                            y_test = y_test)
